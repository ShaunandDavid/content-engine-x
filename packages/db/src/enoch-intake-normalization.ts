...
import type {
  EnochModelRoutingDecision,
  EnochPlanningArtifact,
  EnochReasoningArtifact,
  EnochTextPlanningInput,
  NormalizedIntake,
  ProjectBriefInput,
  PromptDraft,
...
export const normalizeEnochPlanningInput = (input: {
  sourceType: NormalizedIntake["source"]["sourceType"];
  payload: EnochTextPlanningInput;
  routingDecision: EnochModelRoutingDecision;
}): NormalizedIntake => {
...
export const buildNormalizedIntakeFromProjectBrief = (input: {
  ...
  routingDecision: EnochModelRoutingDecision;
  enochPlanningArtifact?: EnochPlanningArtifact | null;
  enochReasoningArtifact?: EnochReasoningArtifact | null;
}): NormalizedIntake => {
  const normalizedFromBrief = normalizeEnochPlanningInput({
  ...
  if (!input.enochPlanningArtifact) {
  ...
  return normalizedIntakeSchema.parse({
    ...
    intent: {
      ...
      coreGoal: input.enochPlanningArtifact.normalizedUserGoal,
      audience: input.enochPlanningArtifact.audience,
      offerOrConcept: input.enochPlanningArtifact.offerOrConcept,
      constraints: input.enochPlanningArtifact.constraints
    },
    planning: {
      requestClassification:
        input.enochReasoningArtifact?.reasoning.requestClassification ?? normalizedFromBrief.planning.requestClassification,
      reasoningSummary:
        input.enochReasoningArtifact?.reasoning.reasoningSummary ?? normalizedFromBrief.planning.reasoningSummary,
      assumptionsOrUnknowns:
        input.enochReasoningArtifact?.reasoning.assumptionsOrUnknowns ?? normalizedFromBrief.planning.assumptionsOrUnknowns,
      recommendedAngle: input.enochPlanningArtifact.recommendedAngle,
      nextStepPlanningSummary: input.enochPlanningArtifact.nextStepPlanningSummary
    }
  });
};
