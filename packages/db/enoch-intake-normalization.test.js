...
const moduleFile = path.join(workspaceRoot, "packages", "db", "src", "enoch-intake-normalization.ts");
...
test("normalizeEnochPlanningInput turns a rough idea into a validated structured intake", () => {
  const module = loadTsModule(moduleFile);

  const intake = module.normalizeEnochPlanningInput({
    sourceType: "rough_idea",
    payload: {
      projectName: "Operator Plan",
      idea:
        "We need Enoch to take a messy campaign idea for our B2B consulting offer and turn it into a tight LinkedIn video brief that our team can execute fast.",
