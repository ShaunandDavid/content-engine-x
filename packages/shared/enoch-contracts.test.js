...
const schemasFile = path.join(workspaceRoot, "packages", "shared", "src", "schemas", "enoch.ts");
const intakeFile = path.join(workspaceRoot, "packages", "shared", "src", "schemas", "intake.ts");
...
test("shared enoch zod contracts accept canonical sample payloads", () => {
  const contracts = loadTsModule(schemasFile);
  const intakeContracts = loadTsModule(intakeFile);

  const parsedRun = contracts.enochRunSchema.parse({
  ...
  const parsedArtifact = contracts.enochArtifactSchema.parse({
  ...
  const parsedRuntime = contracts.enochLangGraphRuntimeStateSchema.parse({
    stateVersion: "enoch.phase0.v1",
  ...
  assert.equal(parsedRuntime.stateVersion, "enoch.phase0.v1");

  const parsedPlanningInput = contracts.enochTextPlanningInputSchema.parse({
    projectName: "Operator Plan",
    idea: "Build a text-first Enoch planning loop that turns rough ideas into a clear operator-ready campaign direction.",
  ...
  const parsedNormalizedIntake = intakeContracts.normalizedIntakeSchema.parse({
    source: {
      sourceType: "rough_idea",
      rawIdea: "Build a text-first Enoch planning loop that turns rough ideas into a clear operator-ready campaign direction."
    },
    intent: {
      projectName: "Operator Plan",
      coreGoal: "Turn rough ideas into a clear operator-ready campaign direction.",
      audience: "Performance marketers",
      offerOrConcept: "Text-first Enoch planning loop",
      constraints: ["Keep it brand safe"],
      tone: "authority"
    },
    delivery: {
  ...
  const parsedPromptGenerationInput = intakeContracts.promptGenerationInputSchema.parse({
    projectName: "Operator Plan",
    coreGoal: "Turn rough ideas into a clear operator-ready campaign direction.",
    audience: "Performance marketers",
    offerOrConcept: "Text-first Enoch planning loop",
  ...
  const parsedPlanningArtifact = contracts.enochPlanningArtifactSchema.parse({
    planId: "44444444-4444-4444-4444-444444444444",
    projectId: "33333333-3333-3333-3333-333333333333",
    workflowRunId: "11111111-1111-1111-1111-111111111111",
    projectName: "Operator Plan",
    sourceIdea: "Build a text-first Enoch planning loop that turns rough ideas into a clear operator-ready campaign direction.",
  ...
  assert.equal(parsedPlanningArtifact.offerOrConcept, "Text-first Enoch planning loop");

  const parsedReasoningArtifact = contracts.enochReasoningArtifactSchema.parse({
  ...
  const parsedVoiceSession = contracts.enochVoiceSessionStateSchema.parse({
    ...
    inputMode: "text",
    outputMode: "text",
    transcript: "What is Enoch's current review status?",
    lastUserMessage: "What is Enoch's current review status?",
    responseText: "Enoch has produced planning, reasoning, and artifacts for this project.",
    errorMessage: null,
  ...
  const parsedVoiceRequest = contracts.enochVoiceRequestSchema.parse({
    projectId: "33333333-3333-3333-3333-333333333333",
    inputMode: "text",
    utterance: "Summarize Enoch's output for this project."
  });
  assert.equal(parsedVoiceRequest.inputMode, "text");

  const parsedVoiceResponse = contracts.enochVoiceResponseSchema.parse({
    session: parsedVoiceSession,
    replyText: "Enoch is ready for review.",
    metadata: {}
  });
  assert.equal(parsedVoiceResponse.replyText, "Enoch is ready for review.");

  const parsedFeedbackRecord = contracts.enochFeedbackRecordSchema.parse({
  ...
  const parsedFeedbackSubmission = contracts.enochFeedbackSubmissionSchema.parse({
    projectId: "33333333-3333-3333-3333-333333333333",
    runId: "11111111-1111-1111-1111-111111111111",
    artifactId: "22222222-2222-2222-2222-222222222222",
    feedbackCategory: "artifact",
  ...
  const parsedRoutingDecision = contracts.enochModelRoutingDecisionSchema.parse({
