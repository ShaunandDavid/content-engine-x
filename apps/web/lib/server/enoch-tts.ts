import type { EnochTtsRequest, EnochTtsResponse } from "@content-engine/shared";
import { enochTtsResponseSchema } from "@content-engine/shared";
...
const getVoicePreferenceScore = (voice: ElevenLabsVoice, preferredVoice: string | undefined, configuredVoiceId: string | undefined) => {
  ...
  if (/(enoch|david)/i.test(voice.name ?? "")) {
    score += 200;
  }
...
export const createEnochTtsResponse = async (request: EnochTtsRequest): Promise<EnochTtsResponse> => {
  ...
  if (!apiKey) {
    return createBrowserSpeechFallback(request, {
      message: configuredVoiceName
        ? `${configuredVoiceName} is selected for Enoch, but server audio is not configured in this runtime yet, so browser speech playback is active.`
        : "Server audio is not configured in this runtime yet, so Enoch is using browser speech playback.",
  ...
  try {
    if (configuredVoiceId) {
      ...
      return enochTtsResponseSchema.parse({
        supported: true,
        ...
        message: spokenTextTruncated
          ? `Server audio is ready through ElevenLabs using ${configuredVoiceName ?? "the configured Enoch voice"} with a shortened spoken version to fit the current ElevenLabs credit budget.`
          : `Server audio is ready through ElevenLabs using ${configuredVoiceName ?? "the configured Enoch voice"}.`,
        metadata: {
          source: "enoch_tts_v1",
          provider: "elevenlabs",
          sessionId: request.sessionId ?? null,
    ...
    if (!selectedVoice) {
      return createBrowserSpeechFallback(request, {
        message: "ElevenLabs is configured, but no usable voices are available to Enoch yet.",
    ...
    return createBrowserSpeechFallback(request, {
      message: quotaDetails
        ? `${selectedVoice?.name ?? configuredVoiceName ?? "Will"} is selected for Enoch, but this ElevenLabs account only has ${quotaDetails.remainingCredits} credits left and this reply needs ${quotaDetails.requiredCredits}, so browser speech playback is active instead.`
        : "ElevenLabs could not finish Enoch audio, so browser speech playback is active instead.",
