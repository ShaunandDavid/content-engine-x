...
const helperFile = path.join(workspaceRoot, "apps", "web", "lib", "server", "enoch-chat.ts");
...
test("createEnochChatResponse persists a real Enoch-created project when the model flags a creation request", async () => {
  const loadTsModule = createTsModuleLoader({
    ...
    "./enoch-project-creation": {
      maybeCreateEnochProjectFromMessage: async () => ({
        matchedIntent: true,
        created: true,
        replyText: 'Created project "Rain Runner". Enoch opened a real Sora planning workflow and saved it to /projects/project-123.',
        provider: "claude",
    ...
    "./enoch-providers": {
      generateEnochReply: async () => {
        throw new Error("generateEnochReply should not run for project creation");
      }
    }
  });
  const module = loadTsModule(helperFile);

  const result = await module.createEnochChatResponse({
    inputMode: "text",
