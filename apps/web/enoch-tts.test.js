...
const helperFile = path.join(workspaceRoot, "apps", "web", "lib", "server", "enoch-tts.ts");
...
const sharedMocks = {
  enochTtsResponseSchema: { parse: (value) => value }
};

test("createEnochTtsResponse uses browser speech fallback when ElevenLabs is not configured", async () => {
  ...
  try {
    const result = await module.createEnochTtsResponse({
      text: "Enoch fallback check."
    });
...
test("createEnochTtsResponse names the configured Will voice when browser speech fallback is active", async () => {
  const loadTsModule = createTsModuleLoader({
    "@content-engine/shared": sharedMocks
  });
  const module = loadTsModule(helperFile);
  ...
  try {
    const result = await module.createEnochTtsResponse({
      text: "Enoch fallback with configured voice."
    });
    ...
    assert.match(result.message, /Will is selected for Enoch/i);
  } finally {
    for (const [key, value] of Object.entries(originalEnv)) {
      if (value === undefined) {
        delete process.env[key];
...
test("createEnochTtsResponse returns audio data when ElevenLabs responds successfully", async () => {
  const originalEnv = {
    ELEVENLABS_API_KEY: process.env.ELEVENLABS_API_KEY,
    ELEVENLABS_VOICE_ID: process.env.ELEVENLABS_VOICE_ID,
    ELEVENLABS_MODEL_ID: process.env.ELEVENLABS_MODEL_ID,
  ...
  try {
    ...
    assert.match(result.message, /configured Enoch voice/i);
  } finally {
...
test("createEnochTtsResponse retries with a shortened spoken version when ElevenLabs quota is exceeded", async () => {
  const originalEnv = {
    ELEVENLABS_API_KEY: process.env.ELEVENLABS_API_KEY,
    ELEVENLABS_VOICE_ID: process.env.ELEVENLABS_VOICE_ID,
    ELEVENLABS_VOICE_NAME: process.env.ELEVENLABS_VOICE_NAME,
  ...
  try {
    ...
    const module = loadTsModule(helperFile);
    const longText =
      "Enoch serves as your project's central intelligence, instantly understanding where your team stands and what needs attention next. " +
